...
function chunkIt(str, length) {
    str = str.replace(/\s{2,}/g,' ')
    const items = str.match(new RegExp('.{1,' + length + '}', 'g'))
...
function NetconfClient(params) {
    ...
    this.rcvBuffer = '';
    this.chunks = ''
    this.debug = params.netconfDebug
    this.sshDebug = params.sshDebug
...
NetconfClient.prototype = {
    ...
    send: function (xml, messageID, callback) {
        const chunked = chunkIt(xml, 982)
        if (this.debug) {
            this.debug(`netconfjs sending: ${chunked}`)
        }
        const self = this;
        this.netconf.write(chunked, function startReplyHandler() {
            // Add an event handler to search for our message on data events.
            self.netconf.on('data', function replyHandler(data) {
                const thisMessage = data.toString().toString().replace(/\n#\d+\n/g, '')
                self.chunks += thisMessage
                if (self.chunks.includes(`message-id="${messageID}`) && self.rcvBuffer.endsWith('</rpc-reply>\n##\n')) {
                    if (self.debug) {
                        self.debug(`netconfjs received: ${chunked}`)
                    }            
                    self.chunks = self.chunks.replace('\n##\n', '')
                    self.parse(self.chunks, callback);
                    self.rcvBuffer = self.rcvBuffer.replace(self.chunks, '');
